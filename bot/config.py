...
class Config(object):
    ...
    # AHCompressBot....
    # sucks Dude
    APP_ID = int(get_config("APP_ID", "20860620"))
    API_HASH = get_config("API_HASH", "25d2343b36fc5aea3604c6c50a8e2b59")
    LOG_CHANNEL = get_config("LOG_CHANNEL", "enclogxx")
    UPDATES_CHANNEL = get_config("UPDATES_CHANNEL", None) # Without `@` LOL
     # Get these values from my.telegram.org
    AUTH_USERS = set(
        int(x) for x in get_config(
            "AUTH_USERS", "6199677027",
            should_prompt=True
        )
    )
# array , simplest method was AUTH_USERS = [] ; AUTH_USERS.append(your telegram id) 🤣
    ...
    TG_BOT_TOKEN = get_config("TG_BOT_TOKEN", "7596475243:AAH8ZPehFqsijjIU7e9TLBmnpek7-wFWlRg")
    # the download location, where the HTTP Server runs
    DOWNLOAD_LOCATION = get_config("DOWNLOAD_LOCATION", "/app/downloads")
    # Telegram maximum file upload size
    BOT_USERNAME = get_config("BOT_USERNAME", "bloodline720p_bot")
