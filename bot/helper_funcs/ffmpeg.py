...
async def convert_video(video_file, output_directory, total_time, bot, message, chan_msg):
    ...
    aa = kk.split(".")[-1]
    out_put_file_name = kk.replace(f".{aa}", "[ENCODED 720p].mkv")
    ...
    audio_b.append("40k")
    watermark.append('-vf "drawtext=fontfile=font.ttf:fontsize=25:fontcolor=white:bordercolor=black@0.50:x=w-tw-10:y=10:box=1:boxcolor=black@0.5:boxborderw=6:text=Anime Sensei"')
    file_genertor_command =  f"ffmpeg -hide_banner -loglevel quiet -progress '{progress}' -i '{video_file}' -metadata 'title=' -c:v {codec[0]}  -map 0 -crf {crf[0]} -c:s copy -pix_fmt yuv420p -s {resolution[0]} -b:v 150k -c:a libopus -b:a {audio_b[0]} -preset {preset[0]} -metadata:s:v 'title=' -metadata:s:a 'title=' -metadata:s:s 'title=' '{out_put_file_name}' -y"
 #Done !!
    ...
    while process.returncode != 0:
      ...
      with open(DOWNLOAD_LOCATION + "/progress.txt", 'r+') as file:
        ...
        stats = f'❄️ ENCODING IN PROGRESS\n\n' \
                f'⌛ TIME LEFT: {ETA}\n\n' \
                f'{progress_str}\n'
        try:
